...
mindepth = 100000000
for i in range(1000):
   z = transpile(qc_from_string,optimization_level=3,basis_gates=['cx','id','rz','sx','x'],seed_transpiler=i)
   if(z.depth()<mindepth):
      mindepth = z.depth()
      print("depth ",z.depth())
      print("opers ",z.count_ops())
      transpiled_circ = z 

np.save("ucc_circ.npy", {"circ":transpiled_circ}, allow_pickle=True)


circ = np.load("ucc_circ.npy", allow_pickle=True).item()
        ...
        for j in range(i+1):
            if calc =="ea":
                ...
                if (i==1 and j==1):
                    ...
                    np.save("QSE_matrixelements_"+calc+"_1.npy", {'H':H, 'S':S}, allow_pickle=True)
