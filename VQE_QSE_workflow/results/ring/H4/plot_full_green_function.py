...
green_full = np.load('qse_green_function_freq_fci.npy',allow_pickle=True)
print (green_full.shape)
green_qse = np.load('qse_green_function_freq.npy',allow_pickle=True)
print (green_full)
a = 4
...
for i in range(a):
     for j in range(a):
         if (i==0 and j==0):
             ...
             axs[0].errorbar(x[p:q],y[p:q],label='re $G_{00}$(FCI)(%d,%d)'%(i,j))
             y  = green_full[:,i,j].imag
             axs[1].errorbar(x[p:q],y[p:q],label='im $G_{00}$(FCI)(%d,%d)'%(i,j))
             x  = x_mesh
             y  = green_qse[:,i,j].real
             axs[0].errorbar(x[p:q],y[p:q],label='re $G_{00}$(QSE)(%d,%d)'%(i,j))
             y  = green_qse[:,i,j].imag
             axs[1].errorbar(x[p:q],y[p:q],label='im $G_{00}$(QSE)(%d,%d)'%(i,j))
...
axs[1].legend()
# ---
axs[0].set_ylabel("Re[$G_{00}$(i$\omega$)]")
