...
def get_results(H_op,A_op,molecule,core,algo_result,outfile,results_with_error_bars=None):
    ...
    if(results_with_error_bars is None):
       ...
       res_vqe = [results_with_error_bars[0][0]+dE,
                  results_with_error_bars[1][0]+dA[0],
                  results_with_error_bars[2][0]+dA[1],
                  results_with_error_bars[3][0]+dA[2]]
       res_vqe_sgm =  [results_with_error_bars[0][1],
                       results_with_error_bars[1][1],
                       results_with_error_bars[2][1],
                       results_with_error_bars[3][1]]

       res_vqe = [(x,y) for (x,y) in zip(res_vqe,res_vqe_sgm)]

       t = PrettyTable(['method','Energy','N','S^2','Sz'])
       t.add_row(['VQE']+[str(round(np.real(x),6))+' +/- '+str(round(np.real(y),6)) for x,y in res_vqe])
       t.add_row(['FCI']+[str(round(np.real(x),6)) for x in res_ee])
       outfile.write(str(t))
       outfile.write("\n")
