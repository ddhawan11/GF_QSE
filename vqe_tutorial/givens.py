...
def givens_operators(G_list,qubit_mapping,two_qubit_reduction,nelec,n):
    givens = []
...
def construct_LO2MO_circuit(VHF,n,nelec,qubit_mapping,two_qubit_reduction):
    n      = VHF.shape[0]
    G_list = decompose_into_givens(VHF)
    G_list = givens_operators(G_list,qubit_mapping,two_qubit_reduction,nelec,n)
    num_qubits = 2*n
    if(two_qubit_reduction): num_qubits -= 2
