...
print("ATHENS RAW --- shots = %d, E = %.6f +/- %.6f " % (8092,res_raw[0],res_raw[1]))
...
#                          measurement_error_mitigation_cls = TensoredMeasFitter, # <===== scalable, but not implemented in qiskit beyond ignis
                           optimization_level               = 0,
                           initial_layout                   = [0,1])
res_1 = measure_operator(H,circ,instance)
print("ATHENS READOUT ERROR MITIGATION (complete) --- shots = %d, E = %.6f +/- %.6f " % (8092,res_1[0],res_1[1]))
